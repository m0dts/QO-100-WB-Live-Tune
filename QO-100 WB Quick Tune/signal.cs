...
namespace QO_100_WB_Quick_Tune
{
    class signal
    {
        ...
        public struct Sig 
        {
            ...
            public Sig(int _fft_start, int _fft_stop,int _fft_centre, int _fft_strength, double _frequency,float _sr)
            {
                ...
                for (i = 2; i < fft_data.Length; i++)
                {
                    ...
                    else /* in_signal == true */
                    {
                        if ((fft_data[i] + fft_data[i - 1] + fft_data[i - 2]) / 3.0 < signal_threshold)
                        {
                            ...
                            if (signal_bw >= 0.033)
                            {
                                signals.Add(   new Sig (start_signal, end_signal, Convert.ToInt32(mid_signal), strength_signal / 255,signal_freq, signal_bw ));
